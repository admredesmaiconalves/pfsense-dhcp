...
if (is_array($dhcpdconf)) {
	...
	$pconfig['class_1'] = base64_decode($dhcpdconf['class_1']); #editado por Maicon Alves 03/06/2020
	$pconfig['custom_1'] = $dhcpdconf['custom_1']; #editado por Maicon Alves 03/06/2020
...
if (isset($_POST['save'])) {
		...
		if (is_array($a_maps)) {
			foreach ($a_maps as $map) {
				...
				if (is_inrange_v4($map['ipaddr'], $_POST['range_from'], $_POST['range_to'])) {
					...
					print_r($map['ipaddr']);
					print_r($_POST['range_from']);
					print_r($_POST['range_to']);
					break;
				}
			}
		}
	...
	if (!$input_errors) {
		...
		$dhcpdconf['class_1'] = base64_encode($_POST['class_1']); #editado por Maicon Alves 03/06/2020
		$dhcpdconf['custom_1'] = $_POST['custom_1']; #editado por Maicon Alves 03/06/2020
...
# editando por Maicon Alves 03/06/2020
if (!is_numeric($pool) && !($act == "newpool")) {
...
$section->addInput(new Form_Input(
	...
	'text',
	$pconfig['custom_1']
))->setHelp('Pool custom field member class. Example: deny members of "mobile"');
#fim da edicao
